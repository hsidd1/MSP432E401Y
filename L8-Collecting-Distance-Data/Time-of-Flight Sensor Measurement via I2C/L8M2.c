...
void rotate(int direction, float stepSize){
	int numFullSteps = (int)(2048.0/360.0*stepSize);
	if(direction == 1){
		for(int j=0;j<numFullSteps/4;j++){
			for(int i=0;i<4;i++){
				GPIO_PORTH_DATA_R = steps[i];
				SysTick_Wait(240000); // 2ms wait
			}
		}
	}else if(direction == -1){
		for(int j=0;j<numFullSteps/4;j++){
  ...
  status = VL53L1X_StartRanging(dev);   // This function has to be called to enable the ranging
    ...
    int average = sum/numSamples;
	  ...
	  status = VL53L1X_ClearInterrupt(dev); // clear interrupt has to be called to enable next interrupt
		...
		sprintf(printf_buffer,"%u ", average);
