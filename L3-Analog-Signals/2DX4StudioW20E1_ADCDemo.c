...
uint32_t func_debug[40]; 											// array used to store realtime data 	
uint32_t ADCavg = 0;


int main(void){
  				
	uint32_t 	delay = 6, 												// this is a multiplier used for SysTick = half of delay 
	...
	while(count < 40){	
		...
		SysTick_Wait(delay * 12000);									// halfdelay*10ms
		func_debug[count] = ADC0_InSeq3();					// Acquire sample	
		if (count <= 9) ADCavg += func_debug[count];// run program for fixed number of steps/samples
		if (count == 10) ADCavg /= 10;
		count++;
	}
