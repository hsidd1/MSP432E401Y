...
void RotateCCW() {                        // Set bits on Port H opposite of Rotate CW in order to spin the motor in the counterclockwise 
...
void RotateCW() {                       	// Function that sets the bits PH0-PH3 that drive the motor clockwise according to fullstep driving
		GPIO_PORTH_DATA_R = 0b00001001;       // First step has a sequence of bits of 1001
		SysTick_Wait(300000);                 // Time waited between each step. This is required as time between steps cannot be 0
...
//  status = VL53L1X_SetInterMeasurementInMs(dev, 200); /* in ms, IM must be > = TB */
  ...
  status = VL53L1X_StartRanging(dev);   /* This function has to be called to enable the ranging */
	...
	for(int stepsCounter = 0; stepsCounter <= 512; stepsCounter++) {
		spin(1);
		if(stepsCounter%64 == 0) {  // Collect a measurement every 45 degrees or 64 steps
